...
- Operational assisted slice: 粗いcueからBase–Foodを完遂し、follow / ignore結果を分離する。deposit成功を有限経験として保持し、2成功後のみcueなしのlearned relationから同じ一周を自律起動できる。generic interrupt候補による一時保留と消失後の再開まで実装済み。Threat / Novelty固有評価はdeferred。
