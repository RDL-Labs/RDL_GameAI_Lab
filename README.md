...
- Observation resolution: [ρ contract](docs/experiment-contracts/RHO_observation_resolution_contract.md)で、同一WorldからLOW / MID / HIGHの有限Food観測差を生成し、版付き契約でNPC別に選択できる。明示設定時だけ通常packetの独立sidecarへ付与し、既存action・canonical pathには未接続。
