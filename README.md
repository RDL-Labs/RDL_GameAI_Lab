...
- Sleep S1-S2: 同一NPCのaccepted raw Experienceを最大6件で固定し、I1/I2のpure compilerでactor / target / context / action / outcomeの有限Profileへ派生する。Nextは[Sleep / Fast-Deep Experience Loop実装計画](docs/design/RDL_GameAI_Sleep_FastDeep循環実装計画.md)のS3 Deep shadow candidateであり、翌朝Fast retrievalをHuntingより先に実装する。
