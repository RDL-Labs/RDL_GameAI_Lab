...
$worldPath = Join-Path $integrationRoot "worlds\rdl_l6"
$runId = [DateTime]::UtcNow.ToString("yyyyMMdd-HHmmss-fff")
$runtimeOut = Join-Path $outputPath "runtime-l6-$runId.stdout.log"
$runtimeErr = Join-Path $outputPath "runtime-l6-$runId.stderr.log"
$luantiOut = Join-Path $outputPath "luanti-l6-$runId.stdout.log"
$luantiErr = Join-Path $outputPath "luanti-l6-$runId.stderr.log"
$luantiLog = Join-Path $outputPath "luanti-l6-$runId.log"
...
try {
    ...
    if (-not $cycleEvidence) { throw "Three L6 outcome cycles were not produced within $TimeoutSeconds seconds" }

    $sleepBody = @{
        agent_id = "npc_a"
        sleep_cycle = "luanti-night-1"
        formation_tick = 40
    } | ConvertTo-Json
    ...
    if (-not $sleep.accepted) { throw "Sleep consolidation was not accepted" }
    if ($sleep.result.status -ne "CANDIDATE_FORMED") { throw "Expected one formed candidate" }
    if ($sleep.result.candidate.support_count -ne 3) { throw "Expected three supporting Profiles" }
    if ($sleep.result.candidate.source_world_event_ids.Count -ne 3) { throw "Expected three World event sources" }
    Write-Output "L6 PASS: candidate=$($sleep.result.candidate.candidate_id) support=3"
} finally {
