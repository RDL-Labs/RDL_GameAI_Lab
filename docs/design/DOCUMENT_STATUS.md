...
| [Sleep / Fast-Deep Experience Loop実装計画](RDL_GameAI_Sleep_FastDeep循環実装計画.md) | active implementation plan v0.1 | 関数階層案の最初の適用先。S1 windowとS2 I1/I2 Profile完了。次はS3 Deep shadow candidate、以後翌朝Fast retrieval、Phase 8 Hunting |
...
| [Sleep Deep Similarity Shadow contract](../experiment-contracts/SLEEP_deep_similarity_shadow_contract.md) | S3 contract frozen / implementation pending | Similarity Observation、one cluster、one shadow candidateを分離。最大6 Profile / 15 pair、coverage・conflict・unresolved非混同、非介入を固定 |
