...
| [Observation System Integration Plan](RDL_GameAI_Observation_System_Integration_Plan.md) | active implementation plan v0.2 | OBS-0〜2完了。個体別SensorProfile、共通frame、隔離store、近景設定化を導入して停止。遠景・聴覚は後続、感覚融合・行動接続・嗅覚は保留 |
| [OBS Common Boundary contract](../experiment-contracts/OBS_common_boundary_contract.md) | OBS-0/1 operational reference | extensionをRuntime入口でlegacy packetから分離し、厳格検証・有限agent/channel store・GET-only snapshotへ隔離。既存意味論・action権限なし |
