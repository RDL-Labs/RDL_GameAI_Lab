...
| [OBS Integrated Life Regression contract](../experiment-contracts/OBS_integrated_life_regression_contract.md) | OBS-6/6C operational reference | RW2生活とA/B別local/distant/auditionを同一World/Runtimeで併走。共通sensor/transmission kernel、一度だけのWorld初期化、tick別window確定、有限配送待ち、壁・対象消失・配送遅延、有限生活互換を固定。action列同一性は未主張 |
