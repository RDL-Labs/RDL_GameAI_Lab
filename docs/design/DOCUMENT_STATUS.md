...
| [感覚観測の解釈・利用への接続計画](RDL_GameAI_Sensory_Interpretation_Use_Plan.md) | OBS-7A diagnostic/replay operational; OBS-7B/8 deferred | OBS-6Eを有限観測基盤として固定。比較適格性・姿勢対応不足・出典・予算・受入試験を定義。候補生成・行動接続は未実装 |
...
| [OBS-7B Auditory Pattern Candidates draft](../experiment-contracts/OBS_7B_auditory_pattern_candidates_contract.md) | design-only / not implemented | 隣接聴覚窓の限定目的、同一取得姿勢、粗い方向・band候補、完全性と候補数の分離、専用実Luanti正例の取得条件 |
