...
| [Observation System Integration Plan](RDL_GameAI_Observation_System_Integration_Plan.md) | active implementation plan v0.2 | OBS-0〜4B完了。個体別profile、隔離store、近景、有限遠景、最小聴覚、動的受信窓を導入。次はOBS-5〜6 read-only表示・統合回帰、感覚融合・行動接続・嗅覚は保留 |
...
| [OBS Audition Receive Window contract](../experiment-contracts/OBS_audition_receive_window_contract.md) | OBS-4B operational supplement | emit時の位置・yawでagent別bufferへ受信し、半開windowを一度だけclose。音源消失・後続回転・遅延配送でも取得時刻/方向を保持し、overflow時も受理済み音を処理 |
