...
| [OBS Integrated Life Regression contract](../experiment-contracts/OBS_integrated_life_regression_contract.md) | OBS-6/6C/6D operational reference | RW2生活とA/B別local/distant/auditionを同一World/Runtimeで併走。共通kernel、一度だけのWorld初期化、tick別window確定、4frame配送、明示ack後除去、拒否・transport失敗時保持、壁・対象消失、有限生活互換を固定。action列同一性は未主張 |
