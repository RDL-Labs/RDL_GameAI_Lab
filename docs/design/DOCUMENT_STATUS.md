...
| [感覚観測の解釈・利用への接続計画](RDL_GameAI_Sensory_Interpretation_Use_Plan.md) | OBS-7A diagnostic / OBS-7B candidates operational; OBS-8 deferred | OBS-6Eを有限観測基盤として固定。比較適格性・姿勢対応不足・出典・予算・受入試験を定義。有限候補生成は検証済み、行動接続は未実装 |
...
| [OBS-8 Visual Reacquisition draft](../experiment-contracts/OBS_8_visual_reacquisition_contract.md) | design-only / not implemented | 同一視覚チャンネル、1回の水平回転、通常周期の再取得、実測姿勢対応・期限・操作再送の契約案。試験未実施 |
