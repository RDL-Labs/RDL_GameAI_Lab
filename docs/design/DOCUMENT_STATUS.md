...
| [NERV-1/2 Neural Gradient contract](../experiment-contracts/NERV_1_2_neural_gradient_projection_contract.md) | design-only / acceptance not run | 二軸の有限作用表、raw不変、知覚勾配とshadow Bias preview。予測誤差・本番Bias・行動接続は含まない |
| [神経](RDL_GameAI_神経パラメーター設計図.md) | current design / status unclear | 操作的ラベル・DNA μ/σ・動的状態・派生感度。design-only |
| [Neural Individuality / Exploration Future Roadmap](RDL_GameAI_Neural_Individuality_Exploration_Future_Roadmap.md) | NERV-0 future roadmap / contract only | 少数の神経感度差から個体差・探索性を創発させる将来順序。NERV-1/2の有限再生契約を設計済み・未実装。NERV-3..9は保留、現行runtime非変更 |
...
| [OBS Audition Receive Window contract](../experiment-contracts/OBS_audition_receive_window_contract.md) | OBS-4B/4C operational supplement | emit時受信、半開window、姿勢保持、遅延配送に加え、window跨ぎ分割、close冪等、late拒否、検出上限のPARTIAL記録を固定 |
| [OBS p5 Sensory Inspection contract](../experiment-contracts/OBS_p5_sensory_inspection_contract.md) | OBS-5 operational reference | agent別・channel別の最新frameを独立時刻、姿勢参照、coverage、欠落状態付きでGET-only表示。時刻同期・意味推定・action権限なし |
| [OBS Integrated Life Regression contract](../experiment-contracts/OBS_integrated_life_regression_contract.md) | OBS-6/6C/6D/6E operational reference | RW2生活とA/B別local/distant/auditionを同一World/Runtimeで併走。共通kernel、一度だけのWorld初期化、tick別window確定、4frame配送、明示ack後除去、拒否・transport失敗時保持、壁・対象消失、有限生活互換を固定。action列同一性は未主張 |
| [感覚観測の解釈・利用への接続計画](RDL_GameAI_Sensory_Interpretation_Use_Plan.md) | OBS-7A diagnostic / OBS-7B candidates operational; OBS-8 finite fixture operational | OBS-6Eを有限観測基盤として固定。比較適格性・姿勢対応不足・出典・予算・受入試験を定義。有限候補生成・専用視覚Probeは検証済み、RW2自律起動は未実装 |
| [OBS-7A Comparison Eligibility contract](../experiment-contracts/OBS_7A_comparison_eligibility_contract.md) | pure diagnostic operational | 取得時間・姿勢・profile・coverageの比較適格性。14試験と実Luanti由来replay。照合・行動・canonical権限なし |
| [OBS-7B Auditory Pattern Candidates contract](../experiment-contracts/OBS_7B_auditory_pattern_candidates_contract.md) | pure candidates / real Luanti replay operational | 隣接聴覚窓の限定目的、同一取得姿勢、粗い方向・band候補、完全性と候補数の分離、専用実Luantiの四状態と18試験。結果は[Evidence](../experiment-evidence/OBS_7B_auditory_pattern_candidates_evidence.md) |
| [OBS-8 Visual Reacquisition contract](../experiment-contracts/OBS_8_visual_reacquisition_contract.md) | finite Luanti probe / pure evaluation operational | 同一視覚チャンネル、1回の水平回転、通常周期の再取得。実測姿勢対応・操作再送を検証。[Evidence](../experiment-evidence/OBS_8_visual_reacquisition_evidence.md)。RW2自律起動は未実装 |
| [OBS-8B Continuous World Probe contract](../experiment-contracts/OBS_8B_continuous_world_probe_contract.md) | continuous-world fixture operational | 実Luanti 6ケース、生活優先・遅延応答・配送ack分離。[Evidence](../experiment-evidence/OBS_8B_continuous_world_probe_evidence.md)。自律起動は未実装 |
| [OBS-9 Observation v1 Completion contract](../experiment-contracts/OBS_9_observation_v1_completion_contract.md) | observation v1 COMPLETE | 全感覚生活＋Probeの実Luanti8run・864frame、受入10条件PASS。[Evidence](../experiment-evidence/OBS_9_observation_v1_completion_evidence.md)。観測能力追加を止め後段研究へ |
